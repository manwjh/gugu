...
final class Affect {
    ...
    /// 0…1 long-term, persisted in state.json
    var bond: Double = 0.1
    /// 0…1 tension
    ...
    func petted()  { valence = min(1, valence + 0.25); bond = min(1, bond + 0.002)
    ...
    func chatted() { bond = min(1, bond + 0.005); valence = min(1, valence + 0.1) }

...
package struct Event: Codable {
    package let time: Date
    package let kind: String      // app_switch / rhythm / poke / petted / dragged / thrown / return / leave / overtime / chat
    package let summary: String   // human-readable line that may be fed to the LLM
    package let weight: Int       // curiosity points
}

/// In-memory queue + JSONL persistence (events/YYYY-MM-DD.jsonl).
/// Main-actor bound: all senses and UI post from the main thread.
@MainActor
package final class EventBus {
    package static let shared = EventBus()
    package private(set) var recent: [Event] = []      // ring buffer of recent events
    package private(set) var curiosity: Double = 0     // accumulates until heartbeat threshold
    package var onPost: ((Event) -> Void)?
    ...
    package func post(kind: String, summary: String, weight: Int) {
    ...
    package func drainCuriosity() { curiosity = 0 }

    /// Recent events formatted for the heartbeat prompt (last N, compact).
    package func promptSummary(limit: Int = 6) -> String {
    ...
    package nonisolated static func lines(for date: Date) -> [String] {
        guard let text = try? String(contentsOf: Paths.eventsFile(for: date), encoding: .utf8) else { return [] }
        return text.split(separator: "\n").map(String.init)
    }

    package nonisolated static func todayLines() -> [String] {
        lines(for: Date())
    }
}

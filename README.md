...
- **记忆与成长**:每天深夜"做梦"整理记忆,把当天见闻蒸馏进 `memory/`,还会总结出自己的行为经验(`skills/`)。第二天它更懂你。
- **对话**:菜单栏"和咕咕说话",它会用小鸟的口吻简短回应。
...
走 `config.yaml` 里的中转通道。心跳用 Haiku(便宜高频),对话用 Sonnet,梦境用 Haiku。

## 调试命令

```bash
swift build -c release
GUGU_HOME=/private/tmp/gugu-offline ./.build/release/gugu --selftest-offline
./.build/release/gugu --selftest          # 真实模型 API 全链路,需要 config.yaml 配好 key
./.build/release/gugu --render happy x.png # 离屏渲染小鸟某个姿态到 PNG
```
...
L3 思考  Sonnet      你主动对话时
L4 梦境  Haiku       每晚整理记忆、生长技能、结算成长
```

设计书全文见 `~/GUGU-DESIGN.md`。

...
| L1 本地物品识别 | 🟡 | 代码完整;模型为可选外挂(`models/gugu-objects.mlmodelc`),缺失时静默跳过 + README 引导 + 审计可查 |
...
| 动作进化(元动作 + 数据化编排 + 学习闭环) | ✅ | 白名单基元沙盒;主人口头教 → 模型出编排 → move_add 提案审批 → 落盘 moves/ |
| 可发现性引导(Discovery) | ✅ | 渐进披露玩法;每条一生一次、每会话至多一条、有冷却,本地零成本 |
| 可见里程碑(Milestones) | ✅ | 互动/羁绊/天数/学会动作跨阈值即庆祝;高水位只触发一次,填补形态进化间的干旱期 |
| idle 行为多样性 | ✅ | 选择抽成纯函数并扩池;受 energy/valence 调制;偶发"自己玩"一个已学动作 |
| 连击戳递进反应 | ✅ | 时间窗连击计数 → 轻跳/烦/晕/躲 四级递进 |
| 择机「灵光」模型(spark) | ✅ | 罕见高价值时机临时借更强模型说一句;每日上限+冷却;默认 spark_id 空=零成本零变化 |
| 预算熔断降档 | ✅ | 三档:全速 / 关对话 / 睡觉 |
| 本地工具:notes / reminders | ✅ | 权限门控 + 审计 |
| 本地工具:web_search | 🟡 | 框架就绪(权限/队列/审计/research 全链路);出网留干净接口,当前仍只记录请求 |
| 定时提醒到点弹通知 | ✅ | 模糊时间词解析为固定时刻,UNCalendarNotificationTrigger 排定 |
...
├── gugu.log             # 本机运行日志
├── memory/              # owner.md / projects.md / self.md(夜间重写);bond.md 大事记(只增)
...
idle ↔ walk ↔ fly ── perch(窗口标题栏)
  ...
  ├── approach / retreat
  ├── peck · dance · stare · groom · yawn
  └── sleep(深夜 / 预算耗尽 / 主动哄睡)
```

L2/L3 只返回**高层动作标签**(enum),具体演出由 L0 决定——模型输出极短,演出质量不依赖模型。
`yawn`(打哈欠:仰头、张嘴、飘一个 z)已是心跳 schema 里可选的动作之一。
...
- 🟡 **本地物品识别**:代码完整,模型是可选外挂。需主人把编译好的 Core ML 模型放到
  `models/gugu-objects.mlmodelc`。缺失时**静默跳过、绝不假装看见**;首启在 `models/`
  生成 README 引导如何启用,且当前启用状态(未安装/已启用/加载失败)写入审计可查。
...
| 大事记 | memory/bond.md | 永续,只增 | 最近 3 条经 `digest()` 进心跳 |
...
**动作不是代码,是数据。** 咕咕"学会"一个新动作 = `moves/` 目录里多出一个 JSON 文件,
而不是新增 Swift 代码再重编译(公理 A1:它=文件在生长)。这条路径让"主人提要求 →
它学会"成为运行时可发生的事,且不破成本与安全底线。

**元动作(meta-action)= 白名单基元沙盒**(`Sources/Gugu/Moves/MetaAction.swift`)。
一个动作被分解成若干步,每步只能调用一张白名单里的身体基元:
`move / rotate / scale / flap / hop / wait / say / view / tilt / blush / peck / groom`。
参数全部被夹到安全区间(位移 ≤120 点、旋转 ≤两圈、单步 ≤2s、整段 ≤8s、≤12 步、台词 ≤24 字)。
因为表达力被**结构性地**限制在"重组已有身体部件"上,模型再怎么写也越不出身体能力边界,
也无法借此越权或作恶——它只能让小鸟动起来(公理 B3)。

**学习闭环**(主人当眼睛,它看不见自己,所以由主人验收——正好落在 B2 的审批权上):

```
主人:"咕咕,学个翻跟头"
  → LearnMoveParser 本地识别这是"教动作"请求(零成本,不烧 token)
  → 已会则直接演;不会则 Brain.learnMove 让模型把意图翻成一段元动作编排(schema 约束只能用白名单基元)
  → 校验/夹紧 → 生成 move_add 提案 → 它开口:"我好像学会「翻跟头」了!你批准我就会了。"
  → 主人在菜单栏批准 → MoveLibrary 再次校验后落盘 moves/翻跟头.json
  → 从此 perform("翻跟头") 命中库即由 MoveInterpreter 翻成 SKAction 演出(本地零成本)
```

- 执行层 `MoveInterpreter` 把 steps 翻成 `SKAction` 跑在 `BirdNode` 上;`PetController.perform`
  在硬编码 switch 之前先查 `MoveLibrary`,学会的动作优先。
- `ProposalEngine` 新增第五类提案 `move_add`,复用快照/审计/归档;恶意编排(未知基元/越界)
  在批准时被校验挡下,绝不落盘。
- 出厂内置三个组合动作作为示范(翻跟头/转圈圈/鞠躬),证明"在元动作之上能长出动作"。
- 右键「动作」子菜单动态列出已学会的动作,点一下即演。

> 边界:数据化编排只能进化"怎么动"(重组现有部件),进化不了"能动什么"——需要新身体部件
> 或新绘制的动作(如"撑伞")仍属开发期路径。基元词汇的丰富度 = 进化空间的上限。
...
- ✅ `notes.add` —— 写本地笔记
- ✅ `reminders.add` —— 记提醒(带 due 时排定定时通知)
- 🟡 `web_search.request` —— **只写一条 pending 记录,不真正联网搜索**

`LocalNotifier` 用 `UNUserNotificationCenter` 发系统通知:无 due 时即时发,带 due 时
经 `DueDateParser` 把模糊时间词解析成固定时刻(今晚=20:00、明天/某日=次日 09:00、
周X=那天 09:00),用 `UNCalendarNotificationTrigger` 到点触发。解析不出的 due 退回即时
通知,绝不静默丢弃提醒。

...
| 夜间自主任务执行 | 🟡 | 队列/调度/审计齐全,但 runner 是占位,不真干活 |
...
`runDue(limit: 5)` 跑到期任务。

> 🟡 **关键限制**:默认 runner 是 `offlineRunner`,对三类任务只返回一句描述字符串
> 就把任务标成 completed——**并不真正写笔记、设提醒或联网研究**。要真干活,需把
> `LocalToolExecutor` 注入为 runner;当前接线没做这一步。所以"夜间自主工作"目前
> 是**形似神缺**:调度、留痕、状态都真,执行是占位。
...
| 夜间真正自主工作 | note/reminder/research 真正执行落地 | runner 是占位(§8.2) |
